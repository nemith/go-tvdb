...
func TestSeriesAllByID(t *testing.T) {
	...
	episodeWant := Episode{
		...
		EpisodeNumber:         1,
		FirstAired:            NewDateStamp(1987, time.April, 19),
		GuestStars:            PipeList{},
		ImdbID:                "",
		Language:              "en",
		...
		ProductionCode:        "101",
		Rating:                NullFloat64{7.0, true},
		RatingCount:           NullInt{1, true},
		SeasonNumber:          0,
		Writer:                PipeList{},
		AbsoluteNumber:        NullInt{},
		BannerFilename:        "episodes/71663/4350173.jpg",
		LastUpdated:           UnixEpochTime(time.Date(2012, time.June, 26, 17, 25, 1, 0, time.UTC)),
		SeasonID:              19130,
		SeriesID:              71663,
		ThumbAdded:            NullDateTimeStamp,
		ThumbHeight:           NullInt{225, true},
		ThumbWidth:            NullInt{300, true},
	}
...
func TestEpisodeBySeries(t *testing.T) {
	...
	for order, ep := range map[string]string{"default": "1/1", "dvd": "1/1", "absolute": "1"} {
		...
		want := &Episode{
			...
			ProductionCode:        "7G08",
			Rating:                NewNullFloat64(7.2),
			RatingCount:           NullInt{},
			SeasonNumber:          1,
			Writer:                PipeList{"Mimi Pond"},
			AbsoluteNumber:        NewNullInt(1),
			BannerFilename:        "episodes/71663/55452.jpg",
			LastUpdated:           UnixEpochTime(time.Date(2011, time.May, 31, 2, 38, 5, 0, time.UTC)),
			SeasonID:              2727,
			SeriesID:              71663,
			ThumbAdded:            NullDateTimeStamp,
			ThumbHeight:           NullInt{300, true},
			ThumbWidth:            NullInt{400, true},
		}
